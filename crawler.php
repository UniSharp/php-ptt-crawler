...
// 取得該board所有文章基本資料
	public function fetch_board($db, $board_name)
	{
		$fe = fopen('php://stderr', 'w');
		$arr_post_list = array();
		$last_page = $this->page_count($board_name, null) + 1;
		fwrite($fe, "total page: " . $last_page . "\n");
		$expired = false;
		$is_last = false;
		for ($i = $last_page; $i >= 1; $i--) {
			fwrite($fe, "fetching page: " . $i . "\n");
			$fetch_data = $this->fetch_item($board_name, $i);
			// 檢測文章是否過期
			if ($expired) {
			...
			foreach ($fetch_data as $item) {
				...
				foreach ($item as $article_arr) {
					$count++;
					if ($count % 4 == 1) {
						$this->save_single_article($db, $board_name, $article_arr);
					}
				}
			}
			sleep(LIST_SLEEP);
...
// 取得該版總頁數
	public function page_count($board_name)
	{
		$result = array();
		$dom = str_get_html($this->fetch_page_html($board_name));
...
// 取得當頁的文章基本資料
	public function fetch_item($board_name, $index)
	{
		$dom = str_get_html($this->fetch_page_html($board_name, $index));
		// 如果取得資料失敗, 回傳NULL
		if ($dom == NULL) {
			return NULL;
		}
...
// 取得當頁的html
	public function fetch_page_html($board_name, $index = null)
	{
		$fe = fopen('php://stderr', 'w');
		$result = null;
...
// 取得當篇文章的html
	public function fetch_article_html($board_name, $id)
	{
		$fe = fopen('php://stderr', 'w');
		$result = null;
...
// 取得當篇文章的詳細資料
	public function fetch_article($board_name, $id)
	{
		$fe = fopen('php://stderr', 'w');
		$dom = str_get_html($this->fetch_article_html($board_name, $id));
		// 如果取得資料失敗, 回傳NULL
		if ($dom == NULL) {
			return $dom;
		}
...
// 存入當篇文章的詳細資料
	public function save_single_article($db, $board_name, $id)
	{
		$fe = fopen('php://stderr', 'w');
		fwrite($fe, "fetching article id: " . $id . "\n");
		$insert_data = $this->fetch_article($board_name, $id);
		// 過濾詭異文章
		if ($insert_data == NULL) {
	...
	public function is_date_over($article_date)
	{
		date_default_timezone_set("Asia/Taipei");
...
$PttCrawler->fetch_board($db, $board_name);
